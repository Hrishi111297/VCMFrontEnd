...
import useProfileImage from "../Main_components/Profile/Util/useProfileImage";
...
const MainHeader = () => {
  ...
  useEffect(() => {
    ...
    if (image) {
      setPreview(image);
      setUserName(authData.user.DATA.firstName+" "+authData.user.DATA.lastName);
    }else{
refreshData();
    }
  }, [image]);
  const handleLogin = () => {
    if (loginHandle === "Logout") {
      ...
      setUserName(authData.user.DATA.firstName+" "+authData.user.DATA.lastName); // Restore user name after login
    }
  };
